...
const probeResponseSchema = z.array(probeItemSchema).min(3).max(5);
const personalizedProbeResponseSchema = z.array(probeItemSchema).min(1).max(5);
...
export function validatePersonalizedProbeResponse(
  raw: unknown
): { valid: true; data: ProbeItem[] } | { valid: false; error: string } {
  const result = personalizedProbeResponseSchema.safeParse(raw);
  if (result.success) {
    return { valid: true, data: result.data };
  }
  return { valid: false, error: result.error.message };
