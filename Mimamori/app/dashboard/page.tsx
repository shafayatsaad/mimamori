...
import type { Probe } from '@/components/ProbeCards';
...
export default function DashboardHomePage() {
  const { currentUserType, currentCaregiverId, caregivers, patientProfile, patientEmail, logs, addLog, updateLog, documents, syncFailed, invitations, hydrationTemperature, hydrationGoal, locationError, aiGenerations, setAIGeneration, getGenerationTriggerHash } = useAppContext();
  ...
  const [checkInRecommendation, setCheckInRecommendation] = useState<{ recommendation: string; severity: 'good' | 'info' | 'warning' } | null>(null);
  ...
  // General daily probes (static, shown on dashboard)
  const GENERAL_PROBES: Probe[] = [
    { title: 'Blood Pressure', question: 'What was your blood pressure reading today?', options: ['Normal (under 120/80)', 'Elevated (120-139/80-89)', 'High (140+/90+)'] },
    { title: 'Sleep Quality', question: 'How did you sleep last night?', options: ['7-9 hours, restful', '5-6 hours', 'Under 5 hours / poor'] },
    { title: 'Medication', question: 'Did you take all your prescribed medication doses today?', options: ['All doses taken', 'Missed one dose', 'Missed multiple'] },
    { title: 'Energy Level', question: 'How is your overall energy level today?', options: ['Good', 'Moderate', 'Very Low'] },
    { title: 'Pain Level', question: 'Are you experiencing any pain or discomfort today?', options: ['None', 'Mild', 'Moderate to Severe'] },
  ];

  // Check which general probes have already been answered today
  ...
  const answeredTitlesToday = new Set<string>();
  for (const log of logs) {
    if (log.date && isLogFromToday(log.date)) {
      if (log.probeAnswers) for (const pa of log.probeAnswers) { if (pa.title) answeredTitlesToday.add(pa.title); }
      if (log.probes) for (const p of log.probes) { const t = p.split(': ')[0]; if (t) answeredTitlesToday.add(t); }
    }
  }
  const unansweredGeneralProbes = GENERAL_PROBES.filter(p => !answeredTitlesToday.has(p.title));

  // Check-in encouragement: show if no logs in the last 24 hours
  const lastLogTime = logs.length > 0 ? new Date(logs[0].date).getTime() : 0;
  const hoursSinceLastLog = logs.length > 0 ? (Date.now() - lastLogTime) / (1000 * 60 * 60) : Infinity;
  const showCheckInNudge = !isCaregiver && hoursSinceLastLog > 24;

  // Answers that indicate a concern and merit follow-up
  const CONCERN_ANSWERS = new Set([
  ...
  const fetchCheckInRecommendation = async (probeAnswers: { title: string; question: string; answer: string }[], logId: string) => {
    setIsGeneratingRec(true);
    try {
      const answersContext = probeAnswers.map(pa => `- ${pa.title}: ${pa.answer}`).join('\n');
      const res = await fetch('/api/medical-reasoning', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          logs: logs.slice(0, 5),
          documents: documents.filter(d => d.analysis).slice(0, 5),
      ...
      console.error('Check-in recommendation generation failed:', err);
    } finally {
      setIsGeneratingRec(false);
    }
  };

  const handleGeneralProbeAnswers = (answers: string[], probeAnswers: { title: string; question: string; answer: string }[]) => {
    const logId = addLog(`Answered ${answers.length} daily check-in probes.`, answers, [], probeAnswers);

    // Only generate a recommendation if any answer indicates a concern
    const hasConcern = probeAnswers.some(pa => CONCERN_ANSWERS.has(pa.answer));
    if (hasConcern) {
      setCheckInRecommendation(null);
      showToast('Checking if follow-up is needed based on your answers...');
      fetchCheckInRecommendation(probeAnswers, logId);
    }
  };
    ...
    <div className="w-full h-full flex flex-col pt-2 max-w-6xl mx-auto relative pb-6 min-w-0">
      ...
      {showCheckInNudge && (
        <motion.div
          ...
          className="mb-8"
        >
          <Link href="/dashboard/daily-log" className="block glass-card rounded-[2rem] p-6 border-2 border-blue-200 bg-blue-50/30 hover:bg-blue-50/60 transition-all group">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-blue-100 text-blue-500 rounded-full flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-blue-900">How are you feeling today?</p>
                <p className="text-xs text-blue-600/70 font-medium mt-0.5">
                  {logs.length === 0
                    ? 'Share how you\'re doing — it helps your care team stay informed.'
                    : 'It\'s been a while since your last check-in. A quick update helps your care team support you better.'}
                </p>
              </div>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-blue-400 flex-shrink-0"><polyline points="9 18 15 12 9 6"/></svg>
            </div>
          </Link>
        </motion.div>
      )}

      {/* Daily General Probes */}
      {!isCaregiver && unansweredGeneralProbes.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.37 }}
          className="mb-8"
        >
          <ProbeCards
            probes={unansweredGeneralProbes}
            onAnswersSubmitted={handleGeneralProbeAnswers}
        ...
        {isGeneratingRec && (
          ...
          >
            <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
              ...
              <span className="text-xs font-medium text-blue-500">Generating follow-up based on your answers...</span>
            </div>
          </motion.div>
        )}
        {!isGeneratingRec && checkInRecommendation && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
          ...
          >
            <div className={`rounded-2xl p-4 flex gap-3 items-start border ${
              checkInRecommendation.severity === 'warning'
                ? 'bg-amber-50 border-amber-200'
                : checkInRecommendation.severity === 'good'
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${
                  checkInRecommendation.severity === 'warning' ? 'text-amber-500' : checkInRecommendation.severity === 'good' ? 'text-emerald-500' : 'text-blue-500'
                }`}>AI Recommendation</p>
                <p className="text-[13px] font-medium text-gray-800 leading-relaxed">{checkInRecommendation.recommendation}</p>
                <p className="text-[10px] text-gray-400 mt-2 italic">AI-generated — not a clinical assessment. Verify with your care team.</p>
              </div>
              <button
                onClick={() => setCheckInRecommendation(null)}
