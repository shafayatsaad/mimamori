...
export default function DailyLogPage() {
  const { logs, addLog, documents } = useAppContext();
  ...
  const fetchProbes = async () => {
    ...
    try {
      const res = await fetch('/api/medical-reasoning', {
        ...
        body: JSON.stringify({ logs: logs.slice(0, 5), documents: documents.slice(0, 5), promptType: 'generate-probes' })
      });
      if (res.ok) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          setDynamicProbes(defaultProbes);
        }
      } else {
        setDynamicProbes(defaultProbes);
      }
  ...
  const handleSaveLog = async () => {
    if (!logText.trim() && selectedProbes.length === 0) return alert('Please enter a log or select a health probe answer.');
    ...
    // Build structured probe answers from selected probes
    const probeAnswers = selectedProbes.map(probe => {
    ...
    addLog(logText || `Answered ${selectedProbes.length} Health Probes today.`, selectedProbes, entities, probeAnswers);

    // Capture answers before clearing state for follow-up generation
    const answeredProbes = [...selectedProbes];

    setLogText('');
    setSelectedProbes([]);
    showToast('Daily log saved with answers. Your care team has been updated.');

    // Auto-generate follow-up probes if the user answered any probes
    if (answeredProbes.length > 0) {
      fetchFollowUpProbes(answeredProbes);
    }
  };

  const fetchFollowUpProbes = async (answeredProbes: string[]) => {
    setIsRefreshingProbes(true);
    try {
      const answersContext = answeredProbes.map(p => `- ${p}`).join('\n');
      const res = await fetch('/api/medical-reasoning', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Append follow-up probes to existing dynamic probes so deduplication still works
          setDynamicProbes(prev => [...prev, ...parsed]);
        } else {
          // No follow-ups needed — generate a recommendation based on all today's answers
          fetchRecommendation(answeredProbes);
        }
      }
    } catch (err) {
  ...
  const fetchRecommendation = async (answeredProbes: string[]) => {
    setIsGeneratingRec(true);
    try {
      // Collect all probe answers from today's logs + the just-answered ones
      const todayAnswers: string[] = [];
      for (const log of logs) {
        if (log.date && isLogFromToday(log.date) && log.probes) {
          todayAnswers.push(...log.probes);
        }
      }
      todayAnswers.push(...answeredProbes);

      const answersContext = [...new Set(todayAnswers)].map(p => `- ${p}`).join('\n');
      const res = await fetch('/api/medical-reasoning', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        try {
          ...
          if (data.insight && data.insight.trim()) {
            setRecommendation({ recommendation: data.insight, severity: 'info' });
          }
        }
      }
    } catch (err) {
  ...
  // Filter out probes already shown/answered today (deduplication - Bug 1.16)
  const filteredProbes = dynamicProbes.filter(
    (probe: any) => !shownProbeTitlesForToday.has(probe.title)
  );
  ...
  return (
    <div className="w-full h-full flex flex-col pt-2 max-w-4xl mx-auto min-w-0">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        >
          Today's AI Health Probes
          {isRefreshingProbes && <span className="text-sm text-gray-400 font-medium ml-2 animate-pulse">Loading...</span>}
        </motion.h3>
        {selectedProbes.length > 0 && (
          ...
          >
            ...
            Save {selectedProbes.length} Answer{selectedProbes.length > 1 ? 's' : ''}
          </button>
        )}
      </div>

      {/* AI Probes Row — with arrow nav on desktop for 3+ probes */}
      <div className="relative">
        {/* Left arrow */}
        ...
        >
          {filteredProbes.length > 0 ? filteredProbes.map((probe: any, idx: number) => {
            const colors = [
               { bg: 'bg-red-50', text: 'text-red-500', hover: 'hover:text-red-900', border: 'border-red-100/50' },
               { bg: 'bg-indigo-50', text: 'text-indigo-500', hover: 'hover:text-indigo-900', border: 'border-indigo-100/50' },
               { bg: 'bg-amber-50', text: 'text-amber-500', hover: 'hover:text-amber-900', border: 'border-amber-100/50' },
               { bg: 'bg-purple-50', text: 'text-purple-500', hover: 'hover:text-purple-900', border: 'border-purple-100/50' },
               { bg: 'bg-emerald-50', text: 'text-emerald-500', hover: 'hover:text-emerald-900', border: 'border-emerald-100/50' }
            ];
            ...
            <div className="flex-shrink-0 w-80 bg-gray-50/50 border border-dashed border-gray-200 rounded-[2rem] p-7 flex items-center justify-center text-gray-400 text-sm font-bold">
               {dynamicProbes.length > 0 && filteredProbes.length === 0
                 ? "All probes answered for today!"
      ...
      {/* Scroll indicator — visible only on mobile when there are multiple probes */}
      ...
      <AnimatePresence>
        {isGeneratingRec && (
          <motion.div
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className="mt-6 bg-blue-50/50 border border-blue-100 rounded-2xl p-5 flex items-center gap-3"
          >
            <div className="w-5 h-5 border-2 border-blue-400 border-t-transparent rounded-full animate-spin flex-shrink-0" />
            <span className="text-sm font-medium text-blue-600">Generating recommendation...</span>
          </motion.div>
        )}
        {recommendation && !isGeneratingRec && (
          <motion.div
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            className={`mt-6 rounded-2xl p-6 flex gap-4 items-start border ${
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className={`text-[10px] font-bold uppercase tracking-widest mb-1.5 ${
                recommendation.severity === 'warning' ? 'text-amber-500' : recommendation.severity === 'good' ? 'text-emerald-500' : 'text-blue-500'
              }`}>
                AI Recommendation
              </p>
              <p className="text-[15px] font-medium text-gray-800 leading-relaxed">{recommendation.recommendation}</p>
            </div>
            <button onClick={() => setRecommendation(null)} className="text-gray-300 hover:text-gray-500 transition-colors flex-shrink-0" aria-label="Dismiss">
